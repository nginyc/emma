...
/**
 * List up to 10 files in google drive
 */
function listFiles() {
 ...
 *
 * @param {String} name name given to new google doc.
 */
function createGoogleDoc(name, text, callback) {
  ...
  fs.readFile('client_secret.json', function processClientSecrets(err, content) {
    ...
    return authorize(JSON.parse(content), (auth) => {
      createDoc(auth, text, name, callback);
    });
  });
}
 ...
 */
function createDoc(auth, text, name='Untitled', callback) {
  var service = google.drive('v3');
  var fileMetadata = {
    'name': name,
